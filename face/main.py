...
from keras.layers import Input, Lambda, Dense, Dropout, Conv2D, MaxPool2D, Flatten
...
def get_data(seize, total_sample_size):
    ...
    for i in range(int(total_sample_size/10)):
        for j in range(10):
            # read images from different direcoty (imposite pair)
            while True:
...
def read_image(filename, byteorder='>'):
    ...
    # then we conver the image to numpy array using np.frombuffer which interprets buffer as one dimensional array
...
def build_base_network(input_shape, nb_filter=[6, 12], kernel_size=3):
    seq = Sequential()
    ...
    seq.add(Conv2D(nb_filter[1], (kernel_size, kernel_size), 
                   padding='valid', data_format="channels_first"))
    seq.add(Activation('relu'))
    seq.add(MaxPool2D(pool_size=(2, 2), data_format="channels_first"))
...
if __name__ == "__main__":
    # img = Image.open("data/orl_faces/s1/1.pgm")
    # img.show()
    ...
    model.fit([img_1, img_2], y_train, validation_split=0.25, batch_size=128, verbose=2, epochs=epochs)
